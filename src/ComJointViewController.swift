...
class ComJointViewController: UIViewController, UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate {
        ...
        Alamofire.request(url, parameters: ["key": "committees"]).responseJSON { (responseData) -> Void in
            if((responseData.result.value) != nil) {
                ...
                if let resData = swiftyJsonVar["results"].arrayObject {
                    ...
                    for item in result {
                        if (item["chamber"] as! String) == "joint" {
                            items.append(item)
                        }
                    }
                    self.arrRes = items
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView == tableViewCom {
            ...
            case 2:
                let comVC = ComJointViewController()
